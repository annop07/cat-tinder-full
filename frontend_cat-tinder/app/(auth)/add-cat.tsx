...
export default function AddCat() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'เพิ่มข้อมูลแมวไม่สำเร็จ');
      }

      // เพิ่มข้อมูลแมวสำเร็จ → ไป home เลย
      router.replace('/(tabs)/home');
    } catch (error: any) {
      console.error('Add cat error:', error);
      Alert.alert('เพิ่มข้อมูลไม่สำเร็จ', error.message || 'กรุณาลองใหม่อีกครั้ง');
      setLoading(false);
    }
  };
